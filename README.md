# Based on the main branch of llm-for-zotero, the LLM chat functionality for Collection categories has been enhanced, allowing the LLM to freely read content within different collections in the right-side panel.
# 基于 llm-for-zotero 的 main 分支，增强了 Collection 分类功能下的 LLM 对话能力，现在可以在右侧面板中自由地让 LLM 读取各个分类的内容。。
