...
import { buildPaperKey, ensurePDFTextCached } from "./pdfContext";
...
import { getSharedZoteroGateway } from "../../agent/index";
...
export async function sendQuestion(
  ...
  try {
    ...
    if (selectedCollections.length > 0) {
      ...
      // Load paper full text from collection (only for Library Chat)
      if (isGlobalPortalItem(item)) {
        try {
          const gateway = getSharedZoteroGateway();
          let totalTokens = 0;
          const paperFullTexts: string[] = [];
          ...
          for (const collectionRef of selectedCollections) {
            ...
            for (const paper of result.papers) {
              ...
              // Try MinerU cache first
              if (isMineruEnabled()) {
                const mineruText = await readCachedMineruMd(paper.itemId);
                if (mineruText) {
                  fullText = mineruText;
                }
              }
              ...
              // Try PDF text cache
              if (!fullText) {
                const pdfContext = pdfTextCache.get(paper.itemId);
                if (pdfContext) {
                  fullText = pdfContext.chunks.join("\n\n");
                }
              }

              // If no cache, try to extract from PDF attachment
              if (!fullText && paper.attachments.length > 0) {
                const contextItemId = paper.attachments[0].contextItemId;
                const attachmentItem = gateway.getItem(contextItemId);
                if (attachmentItem) {
                  await ensurePDFTextCached(attachmentItem);
                  const pdfContext = pdfTextCache.get(attachmentItem.id);
                  if (pdfContext) {
                    fullText = pdfContext.chunks.join("\n\n");
                  }
                }
              ...
              if (fullText) {
                const tokens = estimateTextTokens(fullText);
                totalTokens += tokens;
                const author = paper.firstCreator || "Unknown";
                const year = paper.year || "";
                paperFullTexts.push(
                  `## ${paper.title}\nAuthor: ${author}${year ? ` (${year})` : ""}\n[Estimated tokens: ${tokens}]\n\n${fullText}`,
                );
              } else {
                // Fallback: use abstract if no full text available
                const zoteroItem = gateway.getItem(paper.itemId);
                const abstract =
                  (zoteroItem?.getField?.("abstractNote") as string) || "";
                const author = paper.firstCreator || "Unknown";
                const year = paper.year || "";
                const fallbackText =
                  abstract || "(No full text or abstract available)";
                const tokens = estimateTextTokens(fallbackText);
                totalTokens += tokens;
                paperFullTexts.push(
                  `## ${paper.title}\nAuthor: ${author}${year ? ` (${year})` : ""}\n[No full text - using abstract, tokens: ${tokens}]\n\n${fallbackText}`,
                );
              }
            }

            if (paperFullTexts.length > 0) {
              combinedContext += `\n\nPapers in "${collectionRef.name}" (${result.totalCount} papers, ~${totalTokens} tokens total):\n\n${paperFullTexts.join("\n\n---\n\n")}`;
            }
          }

          // Token warning (only for Library Chat)
          if (totalTokens > 50000) {
            setStatusSafely(
              `Warning: Loading ${totalTokens} tokens from collection papers. Response may be slow.`,
              "warning",
            );
          }
        } catch (err) {
          ztoolkit.log("LLM: Failed to load collection paper fulltexts", err);
        }
      }
    }
