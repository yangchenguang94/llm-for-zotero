...
import type {
  CustomShortcut,
  LibraryChatOutputFormat,
  ReasoningLevelSelection,
} from "./types";
import {
  selectedModelCache,
  panelFontScalePercent,
  libraryChatOutputFormatByLibrary,
} from "./state";
import {
  deriveProviderLabel,
...
const LIBRARY_CHAT_OUTPUT_FORMAT_MAP_PREF_KEY = "libraryChatOutputFormatByLibrary";
...
function getLibraryChatOutputFormatMap(): Record<string, LibraryChatOutputFormat> {
  const raw = getZoteroPrefs()?.get?.(
    `${config.prefsPrefix}.${LIBRARY_CHAT_OUTPUT_FORMAT_MAP_PREF_KEY}`,
    true,
  );
  if (typeof raw !== "string" || !raw.trim()) return {};
  try {
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    const out: Record<string, LibraryChatOutputFormat> = {};
    for (const [key, value] of Object.entries(parsed)) {
      if (value === "synthesis" || value === "per-paper") {
        out[key] = value;
      }
    }
    return out;
...
function setLibraryChatOutputFormatMap(
  value: Record<string, LibraryChatOutputFormat>,
): void {
  getZoteroPrefs()?.set?.(
    `${config.prefsPrefix}.${LIBRARY_CHAT_OUTPUT_FORMAT_MAP_PREF_KEY}`,
    JSON.stringify(value),
...
export function getLibraryChatOutputFormatForLibrary(
  libraryID: number,
): LibraryChatOutputFormat {
  if (!Number.isFinite(libraryID) || libraryID <= 0) return "synthesis";
  const normalizedLibraryID = Math.floor(libraryID);
  const cached = libraryChatOutputFormatByLibrary.get(normalizedLibraryID);
  if (cached) return cached;
  const map = getLibraryChatOutputFormatMap();
  const stored = map[`${normalizedLibraryID}`];
  const resolved = stored === "per-paper" ? "per-paper" : "synthesis";
  libraryChatOutputFormatByLibrary.set(normalizedLibraryID, resolved);
  return resolved;
}

export function setLibraryChatOutputFormatForLibrary(
  libraryID: number,
  format: LibraryChatOutputFormat,
): void {
  if (!Number.isFinite(libraryID) || libraryID <= 0) return;
  if (format !== "synthesis" && format !== "per-paper") return;
  const normalizedLibraryID = Math.floor(libraryID);
  libraryChatOutputFormatByLibrary.set(normalizedLibraryID, format);
  const map = getLibraryChatOutputFormatMap();
  map[`${normalizedLibraryID}`] = format;
  setLibraryChatOutputFormatMap(map);
}
