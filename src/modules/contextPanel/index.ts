        ...
        try {
          ...
          const readerModeLock =
            normalizedReaderLibraryID > 0
              ...
              : null;
          let readerGlobalConversationKey = 0;
          if (readerModeLock === "global" && normalizedReaderLibraryID > 0) {
            // 优先从 Collection session 获取
            ...
            if (collectionContext) {
              ...
              if (collectionKey) {
                readerGlobalConversationKey = collectionKey;
              }
            }
            // Fallback: 从 library session 获取
            if (readerGlobalConversationKey <= 0) {
              readerGlobalConversationKey = Math.floor(
                Number(
