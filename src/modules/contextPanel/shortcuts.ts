...
import type { CustomShortcut, LibraryChatOutputFormat } from "./types";
...
export function closeLibraryChatFormatMenu(body: Element): void {
  const formatMenu = body.querySelector(
    "#llm-library-chat-format-menu",
  ) as HTMLDivElement | null;
  if (!formatMenu) return;
  formatMenu.style.display = "none";
  delete formatMenu.dataset.prompt;
  delete formatMenu.dataset.libraryId;
}
...
export async function renderShortcuts(
  ...
  const inferredMode =
    mode ||
    (panelRoot?.dataset.conversationKind === "global" ? "library" : "paper");
  const isStandaloneBody = (body as HTMLElement).dataset?.standalone === "true";

  if (inferredMode === "library" && isStandaloneBody) {
  ...
  const synthesisOption = body.querySelector(
    "#llm-library-chat-synthesis-option",
  ) as HTMLButtonElement | null;
  const perPaperOption = body.querySelector(
    "#llm-library-chat-perpaper-option",
  ) as HTMLButtonElement | null;
  if (!container) return;
  ...
  const buildLibraryChatPrompt = (
    basePrompt: string,
    format: LibraryChatOutputFormat,
  ): string => {
    const directive =
      format === "per-paper"
        ? "Please analyze the selected papers one by one and organize the response paper by paper."
        : "Please synthesize the selected papers into one integrated analysis that compares and combines their findings.";
    return `${basePrompt}\n\n${directive}`;
  };

  const submitLibraryChatShortcut = (
    prompt: string,
    format: LibraryChatOutputFormat,
  ) => {
    ...
    if (!inputBox || !sendBtn || !item) return;
    const libraryID = Number(
      item.libraryID || panelRoot?.dataset.libraryId || 0,
    );
    if (libraryID > 0) {
      setLibraryChatOutputFormatForLibrary(libraryID, format);
    }
    inputBox.value = buildLibraryChatPrompt(prompt, format);
    closeLibraryChatFormatMenu(body);
    sendBtn.click();
  };

  const openLibraryChatFormatMenu = (
    btn: HTMLButtonElement,
    prompt: string,
  ) => {
    if (!formatMenu || !item) return;
    const libraryID = Number(
      item.libraryID || panelRoot?.dataset.libraryId || 0,
    );
    const currentFormat =
      libraryID > 0
        ? getLibraryChatOutputFormatForLibrary(libraryID)
        : "synthesis";
    formatMenu.dataset.prompt = prompt;
    formatMenu.dataset.libraryId = libraryID > 0 ? `${libraryID}` : "";
    synthesisOption?.setAttribute(
      "aria-pressed",
      currentFormat === "synthesis" ? "true" : "false",
    );
    perPaperOption?.setAttribute(
      "aria-pressed",
      currentFormat === "per-paper" ? "true" : "false",
    );
    positionMenuBelowButton(body, formatMenu, btn);
    formatMenu.style.display = "grid";
  };

  if (formatMenu && synthesisOption && perPaperOption) {
    synthesisOption.textContent = t("Synthesis (All Papers)");
    perPaperOption.textContent = t("Per-Paper Analysis");
    synthesisOption.onclick = (e: Event) => {
      e.preventDefault();
      e.stopPropagation();
      const prompt = (formatMenu.dataset.prompt || "").trim();
      if (!prompt) return;
      submitLibraryChatShortcut(prompt, "synthesis");
    };
    perPaperOption.onclick = (e: Event) => {
      e.preventDefault();
      e.stopPropagation();
      const prompt = (formatMenu.dataset.prompt || "").trim();
      if (!prompt) return;
      submitLibraryChatShortcut(prompt, "per-paper");
    };
  }
  ...
  container.onclick = (e: Event) => {
    ...
    if (inferredMode === "library" && !isStandaloneBody) {
      closeLibraryChatFormatMenu(body); // Close any existing menu first
      openLibraryChatFormatMenu(btn, nextPrompt);
      return;
    }
    const inputBox = body.querySelector(
      "#llm-input",
  ...
  if (menu && menuEdit && menuDelete && menuAdd && menuMove && menuReset) {
    ...
    if (!bodyEl.dataset.llmShortcutBodyClickAttached) {
      ...
      body.addEventListener("click", (e: Event) => {
        ...
        const clickedFormatMenu = Boolean(
          targetEl?.closest("#llm-library-chat-format-menu"),
        );
        menu.style.display = "none";
        menu.dataset.menuKind = "";
        menu.dataset.shortcutId = "";
        menu.dataset.shortcutKind = "";
        if (!clickedShortcutButton && !clickedFormatMenu) {
          closeLibraryChatFormatMenu(body);
        }
        if (
    ...
    // Click listener - registered separately from Escape listener
    if (ownerDoc && !bodyEl.dataset.llmFormatMenuDocClickListenerAttached) {
      bodyEl.dataset.llmFormatMenuDocClickListenerAttached = "true";
      const closeFormatMenuOnDocClick = (e: Event) => {
        const targetEl = e.target as Element | null;
        // Don't close if clicking inside the format menu itself
        const clickedInsideMenu = targetEl?.closest(
          "#llm-library-chat-format-menu",
        );
        // Don't close if clicking a shortcut button (will open new menu)
        const clickedShortcutButton = targetEl?.closest(".llm-shortcut-btn");
        if (!clickedInsideMenu && !clickedShortcutButton) {
          closeLibraryChatFormatMenu(body);
        }
      };
      ownerDoc.addEventListener("click", closeFormatMenuOnDocClick, true);
    }
    // Escape listener (existing logic unchanged)
