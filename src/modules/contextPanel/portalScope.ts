...
export function getSelectedCollectionContext(): CollectionContextRef | null {
  try {
    const pane = Zotero.getActiveZoteroPane?.() as
      | ZoteroPaneWithCollection
      | undefined;
    const selectedCollection = pane?.getSelectedCollection?.() || null;
    if (!selectedCollection) return null;
    const collectionId = normalizePositiveInt(
      selectedCollection.id ?? selectedCollection.collectionID,
    );
    const libraryID = resolveActiveLibraryID();
    if (!collectionId || !libraryID) return null;
    const rawName =
      typeof selectedCollection.getName === "function"
...
export function getSelectedCollectionID(): number | null {
  return getSelectedCollectionContext()?.collectionId || null;
}

export function getSelectedCollectionItemCount(): number | null {
  try {
    const pane = Zotero.getActiveZoteroPane?.() as
      | ZoteroPaneWithCollection
      | undefined;
    ...
    if (
      !selectedCollection ||
      typeof selectedCollection.getChildItems !== "function"
    ) {
      return null;
    }
    const children = selectedCollection.getChildItems(false, true) || [];
    return children.filter((child) => child?.isRegularItem?.()).length;
  } catch (_err) {
    return null;
  }
}

export function isLibraryChatAvailable(): boolean {
  return Boolean(getSelectedCollectionID());
}
...
export function resolveDisplayConversationKind(
  ...
  if (!isGlobalPortalItem(item)) {
    const libraryID =
      normalizePositiveInt(item.libraryID) || resolveActiveLibraryID();
    if (
      libraryID &&
      activeConversationModeByLibrary.get(libraryID) === "global" &&
...
export function resolveInitialPanelItemState(
  ...
  if (
    ...
    let rememberedGlobalKey = Number(
      activeGlobalConversationByLibrary.get(libraryID) || 0,
    );

    // Priority: restore Collection session if exists
    ...
    if (collectionContext) {
      ...
      const collectionKey = activeGlobalConversationByCollection.get(stateKey);
      if (collectionKey) {
        rememberedGlobalKey = collectionKey;
      }
    }

    if (Number.isFinite(rememberedGlobalKey) && rememberedGlobalKey > 0) {
