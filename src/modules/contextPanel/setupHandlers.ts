...
import {
  ...
  activeGlobalConversationByCollection,
...
import {
  ...
  getStringPref,
  getAgentModeEnabled,
  getLibraryChatOutputFormatForLibrary,
  ...
  setLibraryChatOutputFormatForLibrary,
...
import {
  ...
  createGlobalConversationForCollection,
...
import {
  ...
  getSelectedCollectionContext,
  getSelectedCollectionItemCount,
  isGlobalPortalItem,
  isLibraryChatAvailable,
...
import { renderShortcuts, closeLibraryChatFormatMenu } from "./shortcuts";
...
export function setupHandlers(
  ...
  const syncSelectedCollectionScopeForCurrentConversation = () => {
    if (!item) return;
    if (resolveDisplayConversationKind(item) !== "global") return;
    const collectionContext = getSelectedCollectionContext();
    if (collectionContext) {
      selectedCollectionContextCache.set(item.id, [collectionContext]);
      return;
    }
    selectedCollectionContextCache.delete(item.id);
  };

  const updateContextPreviewsVisibility = () => {
    if (!contextPreviews) return;
    // Paper Chat 模式：保持原有显示逻辑不变
    if (!isGlobalMode()) {
      contextPreviews.style.display = "";
      return;
    }
    // Library Chat 模式：新分支，显示 contextPreviews
    // Collection chip 的渲染由 syncSelectedCollectionScopeForCurrentConversation() 处理
    contextPreviews.style.display = "";
  };
  const updateModeChipAvailability = () => {
    if (!modeChipBtn || isStandaloneBody) return;
    const noteSession = resolveCurrentNoteSession();
    const available = isLibraryChatAvailable();
    const inGlobalMode = isGlobalMode();
    modeCapsule?.setAttribute(
      "data-library-chat-available",
      available ? "true" : "false",
    );
    modeChipBtn.disabled = Boolean(!noteSession && !available && !inGlobalMode);
    if (noteSession) return;
    if (!available && !inGlobalMode) {
      const tooltip = t("Select a collection to enable Library chat");
      modeChipBtn.title = tooltip;
      modeChipBtn.setAttribute("aria-label", tooltip);
    }
  };
  const refreshModeDependentUi = async () => {
    syncSelectedCollectionScopeForCurrentConversation();
    updateContextPreviewsVisibility();
    updateModeChipAvailability();
    updatePaperPreview();

    if (item) {
      const shortcutMode = isGlobalMode() ? "library" : "paper";
      await renderShortcuts(body, item, shortcutMode);
    }
  };
  ...
  const syncConversationIdentity = () => {
    ...
    if (item && libraryID > 0 && mode && !noteSession) {
      ...
      if (mode === "global") {
        ...
        // Also remember Collection session
        const collectionContext = getSelectedCollectionContext();
        if (collectionContext) {
          const stateKey = buildCollectionStateKey(
            collectionContext.libraryID,
            collectionContext.collectionId,
          );
          activeGlobalConversationByCollection.set(stateKey, item.id);
        }
        // 注册 Collection 变化监听
        registerCollectionSelectListener();
      } else if (
    ...
    if (modeLockBtn) {
      modeLockBtn.style.display =
        isStandaloneBody && mode === "global" && !noteSession ? "flex" : "none";
      const libraryID = getCurrentLibraryID();
      const lockedKey =
  ...
  // ===== Collection 变化监听 =====
  // 使用 ZoteroPane.collectionsView.onSelect 监听 Collection 选择变化
  // 只在 Library Chat 模式下切换 session，直接调用 switchGlobalConversation，不调用 refreshModeDependentUi
  let collectionSelectListenerRegistered = false;

  const registerCollectionSelectListener = () => {
    if (collectionSelectListenerRegistered) return;
    collectionSelectListenerRegistered = true;

    // 使用 ZoteroPane.collectionsView.onSelect
    const pane = Zotero.getActiveZoteroPane?.() as any;
    if (!pane?.collectionsView?.onSelect?.addListener) {
      ztoolkit.log("LLM: collectionsView.onSelect not available");
      return;
    }

    try {
      pane.collectionsView.onSelect.addListener(async () => {
        // 只在 Library Chat 模式下处理
        ...
        let targetConversationKey =
          activeGlobalConversationByCollection.get(stateKey);

        // 如果没有，创建新 session
        if (!targetConversationKey || targetConversationKey <= 0) {
          try {
            targetConversationKey = await createGlobalConversationForCollection(
              collectionContext.libraryID,
              collectionContext.collectionId,
              collectionContext.name,
            );
            activeGlobalConversationByCollection.set(
              stateKey,
              targetConversationKey,
            );
          } catch (err) {
            ztoolkit.log("LLM: Failed to create collection session", err);
            return;
          }
        }

        // 检查当前 conversation 是否已经是目标 session
        const currentKey = getConversationKey(item);
        if (currentKey === targetConversationKey) return; // 已经是同一个 session

        // 直接切换到新 session（不触发 refreshModeDependentUi）
        await switchGlobalConversation(Math.floor(targetConversationKey));
      });
      ztoolkit.log("LLM: Collection select listener registered");
    } catch (err) {
      ztoolkit.log("LLM: Failed to register collection select listener", err);
    }
  };
  // ===== End Collection 变化监听 =====
  ...
  const appendCollectionChip = (
    ...
    const paperCount = getSelectedCollectionItemCount() || 0;
    const chipLabel = createElement(
      ownerDoc,
      "span",
      "llm-collection-chip-label",
      {
        textContent: `\u{1F5C2}\uFE0F ${ref.name} (${paperCount}篇)`,
        title: `Collection: ${ref.name} - ${paperCount} papers`,
      },
    );
    const removeBtn = createElement(
      ownerDoc,
  ...
  const switchGlobalConversation = async (nextConversationKey: number) => {
    ...
    await refreshModeDependentUi();
    void refreshGlobalHistoryHeader();
  };

  const switchPaperConversation = async (nextConversationKey?: number) => {
    closeLibraryChatFormatMenu(body); // Close library chat format menu when switching to paper chat
    if (!item || isNoteSession()) return;
    persistDraftInputForCurrentConversation();
    const paperItem = resolveCurrentPaperBaseItem();
    if (!paperItem) return;
  ...
  if (modeChipBtn) {
    modeChipBtn.addEventListener("click", (e: Event) => {
      e.preventDefault();
      e.stopPropagation();
      if (isStandaloneBody || isNoteSession()) return;
      if (conversationKey !== null && isRequestPending(conversationKey)) {
        if (status) {
          setStatus(
            status,
      ...
      void (async () => {
        ...
        if (!libraryID) return;
        if (isGlobalMode()) {
          activeConversationModeByLibrary.set(libraryID, "paper");
          await switchPaperConversation();
          return;
        }
        if (!isLibraryChatAvailable()) {
          updateModeChipAvailability();
          if (status) {
            setStatus(
              status,
              t("Select a collection to enable Library chat"),
              "warning",
            );
          }
          return;
        }
        let targetConversationKey: number | undefined;
        const collectionContext = getSelectedCollectionContext();
        if (collectionContext) {
          const stateKey = buildCollectionStateKey(
            collectionContext.libraryID,
          ...
          if (
            ...
            try {
              targetConversationKey =
                await createGlobalConversationForCollection(
                  collectionContext.libraryID,
                  collectionContext.collectionId,
                  collectionContext.name,
                );
              ...
              ztoolkit.log(
                "LLM: Failed to create collection chat conversation",
                err,
              );
            }
          }
        ...
        if (
          ...
          if (
            ...
            try {
              ...
              ztoolkit.log(
                "LLM: Failed to create library chat conversation",
                err,
              );
            }
          }
        ...
        const selectedCollectionCount = getSelectedCollectionItemCount();
        if (selectedCollectionCount === 0 && status) {
          setStatus(status, t("Current collection has no papers."), "warning");
        }
        await switchGlobalConversation(Math.floor(targetConversationKey));
      })();
    });
