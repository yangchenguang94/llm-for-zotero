...
export async function createGlobalConversationForCollection(
  libraryID: number,
  collectionId: number,
  ...
  const title = collectionName ? `${collectionName}` : "Collection Chat";
  const now = Date.now();
  ...
  return await Zotero.DB.executeTransaction(async () => {
    await Zotero.DB.queryAsync(
      `INSERT INTO ${GLOBAL_CONVERSATIONS_TABLE}
       (conversation_key, library_id, created_at, title)
       VALUES ((SELECT COALESCE(MAX(conversation_key), ${GLOBAL_CONVERSATION_KEY_BASE - 1}) + 1 FROM ${GLOBAL_CONVERSATIONS_TABLE}), ?, ?, ?)`,
      [normalizedLibraryID, now, title],
    );

    const rows = (await Zotero.DB.queryAsync(
      `SELECT MAX(conversation_key) AS conversationKey
       FROM ${GLOBAL_CONVERSATIONS_TABLE}`,
    )) as Array<{ conversationKey?: unknown }> | undefined;

    return Number(rows?.[0]?.conversationKey) || 0;
  });
}
