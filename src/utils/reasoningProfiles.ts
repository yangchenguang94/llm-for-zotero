...
> = {
  ...
  deepseek: {
    rules: [
      {
        // 官方 DeepSeek：deepseek-reasoner, deepseek-r1
        match: /^deepseek-(?:reasoner|r1)(?:\b|[.-])/i,
        profile: DEEPSEEK_REASONER_PROFILE,
      },
      {
        // 硅基流动 DeepSeek-R1 系列（支持 Pro/ 前缀）
        match: /^(?:Pro\/)?(?:deepseek-ai\/)?DeepSeek-R1/i,
        profile: DEEPSEEK_REASONER_PROFILE,
      },
      {
        // 硅基流动 DeepSeek-V3 系列 - 强制开启思考模式（支持 Pro/ 前缀）
        match: /^(?:Pro\/)?(?:deepseek-ai\/)?DeepSeek-V3/i,
        profile: {
          supportsReasoning: true,
          defaultLevel: "default",
          options: [option("default", "enabled")],
      ...
      {
        // 硅基流动其他 DeepSeek 模型 - 默认开启思考（支持 Pro/ 前缀）
        match: /^(?:Pro\/)?(?:deepseek-ai\/)?DeepSeek-/i,
        profile: {
          supportsReasoning: true,
          defaultLevel: "default",
          options: [option("default", "enabled")],
